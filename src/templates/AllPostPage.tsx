...
export default function AllPostPageTemplate({
  data,
}: AllPostPageTemplateProps) {
  ...
  return (
    <MainLayout>
      ...
      {pageCount > 1 && (
        <Pagenation currentPage={currentPage} pageCount={pageCount} />
      )}
      {/* <Profile /> */}
    </MainLayout>
  );
...
export const Head: HeadFC<Queries.AllPostPageTemplateQuery> = ({ data }) => {
  const title = "앱공방 - 1인 개발 프리랜서";
  const description = "프리랜서 개발자 앱공방입니다.";
