...
export const Head: HeadFC<Queries.PostPageQuery> = ({ data }) => {
  const locale = "ko";
  const title = `${data.post?.frontmatter?.title!} - 앱공방 기술 블로그`;
  const description = data.post?.frontmatter?.description!;
  const ogimage =
    data.post?.frontmatter?.thumbnail?.childImageSharp?.gatsbyImageData!;
  ...
  return (
    <>
      ...
      <meta
        property="og:url"
        content={`${DOMAIN}/posts/${data.post?.frontmatter?.slug}`}
      />
      ...
      <meta
        name="twitter:data1"
        content={`${devCategory} | ${data.post?.frontmatter?.tags![0]}`}
      />
      <meta
        name="article:published_time"
        content={`${data.post?.frontmatter?.createdAt?.replace(
          /[/]/g,
          "-"
        )}T09:00:00.000Z`}
      />
